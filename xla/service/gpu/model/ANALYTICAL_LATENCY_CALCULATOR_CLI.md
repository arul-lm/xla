...
| `--hierarchical-allreduce-enabled` | **Calcium-only (Step 9).** Tri-state selector for the hierarchical AllReduce cost model. Default **`-1`** = leave at the built-in OFF default (flat worst-pair AR; byte-stable). **`0`** = explicit OFF (flat worst-pair AR). **`1`** = ON (hierarchical `{Pod, Card, Server}` for q250, `{Pod}` or `{Pod, Rack}` for q250l200). **No `.config` key** exists — this flag (or the matching FFI parameter) is the only way to turn the model on. Silently ignored on non-Calcium archs. |
...
| `--hierarchical-allreduce-enabled=H` | **(Step 9)** Tri-state selector for the Calcium hierarchical AllReduce cost model. `-1` (default) leaves the model at its built-in OFF default (flat worst-pair AR; byte-stable). `0` = explicit OFF. `1` = ON (hierarchical `{Pod, Card, Server}` for q250, `{Pod}` or `{Pod, Rack}` for q250l200). Affects only `kAllReduce`/`kAllReduceStart` opcodes. **No `.config` key** exists — this flag (or the matching FFI parameter) is the only way to turn the model on. Silently ignored on non-Calcium archs. See [`cebuq/HIERARCHICAL_ALLREDUCE_PLAN.md`](../../../cebuq/HIERARCHICAL_ALLREDUCE_PLAN.md). |
