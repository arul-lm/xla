...
int main(int argc, char *argv[]) {
  ...
  absl::Status validation_status =
      ValidateHardwareArchitectures(opts.hardware_architectures);
  ...
  for (const auto &pair : gpu_specs) {
    ...
    if (opts.dump_modified_module && !opts.output_dir.empty()) {
      std::string hlo_output_path = tsl::io::JoinPath(
          tsl::io::JoinPath("/xla", opts.output_dir), "modified_module.hlo");
    ...
    auto cluster_config = GetClusterConfigByName(spec_file_name);
    if (!cluster_config) {
        ...
        case xla::HloOpcode::kWhile: {
          ...
          if (while_config.ok() && while_config->has_known_trip_count()) {
            ...
            if (computed_trip_count.has_value()) {
              ...
              llvm::errs() << "ERROR: Could not determine trip count for while loop: "
                           << instr->name() << "\n";
              llvm::errs().flush();
              return -1;
            }
